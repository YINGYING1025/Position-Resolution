...
double x_residual=0.;
double y_residual=0.;
const int xAligbinNum=96;//AllEvent & Aligment 
const int yAligbinNum=1100;

TCanvas *c4=new TCanvas("c4","All_Aligment_PR",1000,500);
TH1F *xAlig =new TH1F("xAlig","AllMid AligmentX PR",xAligbinNum,-3,3);
TH1F *yAlig =new TH1F("yAlig","AllMid AligmentY PR",yAligbinNum,-50,50);
TH1F *h1 =new TH1F("h1","AllMid NoAligmentX PR",xAligbinNum,-3,3);
double x_resi=0.;
vector<double>x1;
...
//	arglist[0]=2;
	...
	double step[npar] = {0.000001, 0.001};
	...
	//detaX = 0.10;
	for(int j=0; j<x2.size(); j++){
		val+=pow((35.3/66.2*x1[j]+30.9/66.2*x3[j])*cos(angle)-(46.1/66.2*y1_sTGC[j]+20.1/66.2*y3[j])*sin(angle)+detaX-x2[j],2);
	}
	cout<<"the manual fcn value = "<<val<<endl;
	 for(int j=0; j<x2.size(); j++){
		...
		//x_residual=(35.3/66.2*x1[j]+30.9/66.2*x3[j])*cos(angle)-(46.1/66.2*y1_sTGC[j]+20.1/66.2*y3[j])*sin(angle)+detaX-x2[j]-77*detaZ/(x1[j]-x3[j]);
		 ...
		 x_resi=35.3/66.2*x1[j]+30.9/66.2*x3[j]-x2[j];
		 h1->Fill(x_resi);
        }
	delete gMinuit;
	c4->Divide(2,1);
	...
	xAlig->GetXaxis()->SetTitle("mm");
        ...
        xAlig->GetXaxis()->SetRangeUser(-3,3);
          TAxis *xaxis =  xAlig->GetXaxis();
	int a=0;
	for(int i=0;i<96;i++){
	Double_t binContent = xAlig->GetBinContent(i);
	if(binContent==0) a++;
	}
	 cout<<" xAlig ContentBin=0 Number is: "<<a<<endl;
	xAlig->Fit("gaus","","",-3,3);
        xAlig->Draw();
	c4->cd(2);
	h1->GetXaxis()->SetTitle("mm");
        ...
        int b=0;
	 TAxis *yaxis = h1->GetXaxis();
        for(int i=0;i<96;i++){
         Double_t ybinContent = h1->GetBinContent(i);
        if(ybinContent==0) b++;
	}
	cout<<" yAlig ContentBin=0 Number is: "<<b<<endl;
	h1->Fit("gaus","","",-3,3);
	h1->Draw();
	return 0;
}
