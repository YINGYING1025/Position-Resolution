...
TFile *file1 = new TFile("ThreeSumADC_20012.root","RECREATE");
...
void Position_20012(){
	TFile *file = new TFile("XYposition_20012.root","RECREATE");
	...
	while(getline(data,stor,'\n')){
		...
		//	cout<<endl;}
			...
			if(Event>26168) break;;
		 
		}
		if(Event!=26168) continue;
		//if(Event>10) break;
		//@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@//
		if(line_size==54){
			if(Pulse==1){
...
void position(){
	if(UpStripCount>Strip_Cut&&UpStripCount_y>Strip_Cut&&MidStripCount>Strip_Cut&&MidStripCount_y>Strip_Cut&&DownStripCount>Strip_Cut&&DownStripCount_y>Strip_Cut){      
		...
		for(int i=0; i<UpStripCount;i++){
			...
			cout<<"Event x1 strip number: "<<Upstrip[i]<<" sumADC: "<<Upsumadc[i]<<endl;
		}
		x1=WeightQ1/Q1;
		//Y1
		for(int i=0; i<UpStripCount_y;i++){
			...
			cout<<"Event y1 strip number: "<<Upstrip_y[i]<<" sumADC: "<<Upsumadc_y[i]<<endl;
		}
		...
		for(int i=0; i<MidStripCount;i++){
			...
			cout<<"Event x2 strip number: "<<Midstrip[i]<<" sumADC: "<<Midsumadc[i]<<endl;
		}
		x2=WeightQ2/Q2;
		//Y2
		...
		for(int i=0; i<MidStripCount_y;i++){
			...
			cout<<"Event y2 strip number: "<<Midstrip_y[i]<<" sumADC: "<<Midsumadc_y[i]<<endl;
		}
		y2=WeightQ2_y/Q2_y;
		...
		for(int i=0; i<DownStripCount;i++){
			...
			h3x->SetBinContent(Downstrip[i],Downsumadc[i]);
			 cout<<"Event x3 strip number: "<<Downstrip[i]<<" sumADC: "<<Downsumadc[i]<<endl;
		}
		x3=WeightQ3/Q3;
		//cout<<"Event x3: "<<y3<<endl;
		//Y3:
		for(int i=0; i<DownStripCount_y;i++){
			...
			cout<<"Event y3 strip number: "<<Downstrip_y[i]<<" sumADC: "<<Downsumadc_y[i]<<endl;
		}
		y3=WeightQ3_y/Q3_y;
		//cout<<"Event y3: "<<y3<<endl;
