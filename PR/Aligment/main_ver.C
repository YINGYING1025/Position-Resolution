...
int main_ver(){
	...
	//double detaX = 0.298332;
	//double angle = 0;
	//double detaX = 0.;
	for(int i=0;i<50;i++) cout<<"*"; cout<<endl;
	cout<<"The number is: "<<x2.size()<<endl;
	for(int i=0;i<50;i++) cout<<"*"; cout<<endl;

	const int n_residual=1009;
	double resolution[n_residual]={0.};
	double EventNum[n_residual]={0};
	for(int j=0; j<x2.size(); j++){
	...
	TGraph* gr = new TGraph(n_residual,EventNum,resolution);
	TCanvas *c1 = new TCanvas("c1","Sigma_Angle",200,10,500,300);
	c1->cd();
	c1->SetGrid();
	gr->SetLineWidth(2);
	gr->GetXaxis()->SetTitle("EventNum");
	gr->GetYaxis()->SetTitle("Resolution");
	gr->SetTitle("EventNum_Resolution");
	gr->SetMarkerStyle(4);
	gr->SetLineColor(2);
	gr->Draw("APL");
	c1->SaveAs("EventNum_Resolution");
