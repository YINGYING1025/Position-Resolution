...
void fcn12x3(int &npar, double *gin, double &chi2, double *par, int iflag){
	...
	double detaZ = par[2];
		...
		//if(x1[j]-x3[j]!=0){
               // chi2+=pow((x1[j]+x3[j])/2*cos(angle)-(y1_sTGC[j]+y3[j])/2*sin(angle)+detaX-x2[j]-66.2*detaZ/(x1[j]-x3[j]),2);}
		chi2+=pow(((35.3+detaZ)/66.2*x1[j]+(30.9-detaZ)/66.2*x3[j])*cos(angle)-((46.1+detaZ)/66.2*y1_sTGC[j]+(20.1-detaZ)/66.2*y3[j])*sin(angle)+detaX-x2[j],2);
        //}
        chi2 *=2;
...
int main3(){
	...
	for(int i=0;i<nentries;i++){
               ...
               cout<<px1<<" "<<px2<<" "<<px3<<" "<<py1<< " "<<py2<<" "<<py3<<endl;
...
//	arglist[0]=2;
	...
	for(int j=0; j<x2.size(); j++){
		...
		//val+=pow((x1[j]+x3[j])*(1/2)*cos(angle)-(y1_sTGC[j]+y3[j])*(1/2)*sin(angle)+detaX-x2[j]-66.2*detaZ/(x1[j]-x3[j]),2);
		val+=pow(((35.3+detaZ)/66.2*x1[j]+(30.9-detaZ)/66.2*x3[j])*cos(angle)-((46.1+detaZ)/66.2*y1_sTGC[j]+(20.1-detaZ)/66.2*y3[j])*sin(angle)+detaX-x2[j],2);
	}
	cout<<"the manual fcn value = "<<val<<endl;
	 for(int j=0; j<x2.size(); j++){
		...
		//angle=0;
		// detaX=0;
		 //x_residual=(x1[j]+x3[j])/2*cos(angle)-(y1_sTGC[j]+y3[j])/2*sin(angle)+detaX-x2[j]-66.2*detaZ/(x1[j]-x3[j]);
		 x_residual=((35.3+detaZ)/66.2*x1[j]+(30.9-detaZ)/66.2*x3[j])*cos(angle)-((46.1+detaZ)/66.2*y1_sTGC[j]+(20.1-detaZ)/66.2*y3[j])*sin(angle)+detaX-x2[j];
